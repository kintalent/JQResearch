# 克隆自聚宽文章：https://www.joinquant.com/post/4379
# 标题：面向对象重构策略(二):次新小盘策略
# 作者：晚起的小虫
...
def select_strategy():
    ...
    g.adjust_condition_config = [
        ...
        #         'minute' : 50                   # 调仓时间，分钟
        #     }],
        # [False,'28择时','Index28_condition',{    # 该调仓条件可能会产生清仓行为
        #         'index2' : '000016.XSHG',       # 大盘指数
        #         'index8' : '399333.XSHE',       # 小盘指数
        #         'index_growth_rate': 0.01,      # 判定调仓的二八指数20日增幅
        #     }],
    ...
    g.filter_stock_list_config = [
        ...
        # [False,'过滤涨停,不过滤持仓股','Filter_limitup',{}],
        # [False,'过滤跌停,不过滤持仓股','Filter_limitdown',{}],
        [True, '过滤上市超过一定时间的股票', 'Filter_old_stock', {
            'day_count': 365  # 超过多少天
        }],
        [True, '过滤涨跌停,持仓股也过滤', 'Filter_limit_up_and_down', {}],
...
class Index28_condition(Adjust_condition):
    ...
    def handle_data(self, context, data):
        ...
        log.info("当前%s指数的20日涨幅 [%.2f%%]" % (get_security_info(self.index2).display_name, gr_index2 * 100))
        log.info("当前%s指数的20日涨幅 [%.2f%%]" % (get_security_info(self.index8).display_name, gr_index8 * 100))
...
class Filter_limit_up_and_down(Filter_stock_list):
    __name__ = 'Filter_limit_up_and_down'
...
class Stat(Rule):
    ...
    def when_sell_stock(self, position, order, is_normal):
        if order.filled > 0:
            ...
            self.trade_stat.watch(position.security, order.filled, position.avg_cost, position.price)
